...
function mealFeed() {
  ...
  `;
    for(let i = (curPage - 1) * 10; i < curPage * 10; i++)mealList.push(`
  ...
  `);
    ...
    template = template.replace("{{__prev_page__}}", curPage > 1 ? curPage - 1 : 1);
    template = template.replace("{{__next_page__}}", curPage >= mealFeed.meals.length % 10 ? curPage : curPage + 1);
...
function mealDetail() {
  ...
  `;
    ...
    template = template.replace("{{__meal_ingredient__}}", ingredientList.join(""));
    container.innerHTML = template;
}
function router() {
    const routePath = location.hash;
    if (routePath === "") mealFeed();
